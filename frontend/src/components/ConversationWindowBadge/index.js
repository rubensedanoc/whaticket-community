...
const ConversationWindowBadge = ({ conversationWindow, isGroup }) => {
  ...
  if (isGroup) {
    return (
      <Chip
        size="small"
        label="Grupo — Sin restricción Meta"
  ...
  if (isOpen && type === "active") {
    ...
    if (remaining < 2) {
      return (
        <Chip
          size="small"
          label={`Ventana Meta por cerrar · Quedan ${formatTime(remaining)}`}
    ...
    return (
      <Chip
        size="small"
        label={`Ventana Meta activa · Quedan ${formatTime(remaining)}`}
  ...
  if (type === "new_contact") {
    return (
      <Chip
        size="small"
        label="Ventana Meta · Sin historial — Se usará plantilla"
  ...
  if (type === "expired") {
    ...
    return (
      <Chip
        size="small"
        label={`Ventana Meta cerrada${hoursAgo ? ` · Último msg: hace ${hoursAgo}` : ""}`}
        className={classes.badgeClosed}
      />
    );
  }
